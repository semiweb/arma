...
#call this function to update commit database with new local git commits
  def self.parse_git_log(application_path,application_id, commit_SHA= '')
    ...
    last_commit  = Commit.where(imported_gitlog: true).order(:created_at).last
    ...
    commits.each do |c|
      ...
      if p.changelogs.present? || commit_SHA.present?
        ...
        new_commit.update_attributes(commit_time: commit_array[1], full_commit_message: commit_message, application_id: application_id, branch: branch, author: commit_array[2], downtime: p::annotations.include?('downtime'), imported_gitlog: true )
